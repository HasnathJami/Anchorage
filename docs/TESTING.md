...
| `device_matrix_test.dart` | 24 | Both screens laid out across four device shapes and two text scales. Not goldens: it asserts the two things a design built against one phone actually gets wrong elsewhere — an overflow, and a control pushed off the bottom of a short screen |
